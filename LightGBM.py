...
# Import LightGBM core modules and early stopping callback
...
def objective(params):
    ...
    # Initialize LightGBM regression model (verbose is set during initialization, not in fit method)
    model = LGBMRegressor(
        ...
        verbose=-1  # Silent mode (only set during model initialization)
    )

    # Core correction: Remove verbose=False from fit method (fit does not support this parameter)
    model.fit(
        X_train, y_train,
        eval_set=[(X_validation, y_validation)],
        eval_metric='rmse',
        callbacks=[early_stopping(stopping_rounds=50, verbose=False)]  # Keep early stopping callback
    )
...
for i in range(n_bootstrap):
    ...
    model_bs = LGBMRegressor(**best_params)
    model_bs.fit(X_bs, y_bs)  # No additional parameters for fit here (naturally supported)
...
best_model.fit(X_train, y_train)  # No additional parameters for fit here
...
test_df.to_csv(os.path.join(output_dir, 'IMERG-E-lightgbm-bootstrap-residual.csv'), index=False)
...
importance_df.to_csv(
    os.path.join(output_dir, 'IMERG-E-lightgbm-shap_importances.csv'),
    index=False
)
...
plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, 'E-LightGBM_SHAP_top10_combined.png'),
    dpi=900
)
plt.show()
...
plt.ylabel('Bias')
plt.title(f'Observed vs Predicted Bias (LightGBM) with 95% CI\nCoverage: {within_ci:.2f}%')
plt.legend()
plt.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'E-LightGBM_prediction_comparison_CI.png'), dpi=900)
plt.show()

print(f"✅ LightGBM full workflow completed, results saved to: {output_dir}")